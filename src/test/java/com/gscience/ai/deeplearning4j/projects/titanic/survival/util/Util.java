...
public class Util {
    ...
    /**
     * Get the resource URI of a file in the resource dir
     * @param resourceMapAndFile
     * @return
     * @throws FileNotFoundException
     * @throws URISyntaxException
     */
    private static URI getResourceURL(String resourceMapAndFile) throws FileNotFoundException, URISyntaxException {

        // 1. Static-safe way to get the resource
        URL res = Util.class.getClassLoader().getResource(resourceMapAndFile);

        if (res == null) {
            throw new java.io.FileNotFoundException("Resource not found at: src/main/resources/" + resourceMapAndFile);
        }
     ...
     * @throws Exception
     */
    public static Dataset<Row> getTrainingDF() throws Exception {

        // 2. Convert to URI then Path to handle Windows file systems correctly
        String absolutePath = new java.io.File(getResourceURL("data/titanic.survival.predection/Titanic_train.csv")).getAbsolutePath();
        ...
        // This prints the first 20 rows to the console
        df.show();

        // Highly recommended: This shows you if Spark correctly identified
        // "Survived" as an Integer and "Age" as a Double
        df.printSchema();

        System.out.println("Feature Selectioation and Training data transformation");
        ...
        // Show the first 20 rows of the processed data
        projection.show();

        // Verify that Sex and Embarked are now numeric types (Double or Integer)
        projection.printSchema();

        // Create dense vector use for linear algebra
        // for calculation of statics function
        ...
        // take(5) pulls the first 5 records from the cluster to your console
        // calculating the baseline
        statsDf.take(5).forEach(vector -> {
            System.out.println("Feature Vector: " + vector.toString());
        });

        // find the shape of your future
        // heave parralell calculation
        summary = Statistics.colStats(statsDf.rdd());

        // the average value
        // Example: Is the average Fare $32 or $500?
        double meanFare = summary.mean().apply(0);

        // the average value of the age
        ...
        System.out.println("Fill in the missing value with average values");
    ...
    public static Dataset<Row> getTestDF() throws FileNotFoundException, URISyntaxException {
        spark.sqlContext().udf().register("normSex", normSex, DataTypes.IntegerType);
        spark.sqlContext().udf().register("normEmbarked", normEmbarked, DataTypes.IntegerType);

        // FIX: Get the absolute path using your helper method
        String absolutePath = new File(getResourceURL("data/titanic.survival.predection/Titanic_test.csv")).getAbsolutePath();
